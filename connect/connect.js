...
function check(player) {
  setTimeout(() => {
    ...
    for (i = 1; i <= 4; i++) {
      for (j = 6; j >= 4; j--) {
        if (
          document.getElementById(`c${i}r${j}`).style.backgroundColor == `${player}` && document.getElementById(`c${i + 1}r${j - 1}`).style.backgroundColor ==`${player}` && document.getElementById(`c${i + 2}r${j - 2}`).style.backgroundColor == `${player}` && document.getElementById(`c${i + 3}r${j - 3}`).style.backgroundColor ==`${player}`) {
          alert(`${player} wins`);
          location.reload();
        }
      }
...
document.querySelectorAll(".column").forEach((e) => {
  e.addEventListener("click", () => {
    ...
    } else if (sum <= 6 && turn % 3 == 1) {
      document.getElementById(`${e.id}r${sum}`).style.backgroundColor =
        "yellow";
      turn++;
      check("yellow");
      document.getElementById("whosturn").innerText = "Green's Turn";
    } else if (sum <= 6 && turn % 3 == 2) {
      document.getElementById(`${e.id}r${sum}`).style.backgroundColor =
        "green";
      turn++;
      check("green");
